...
public class Main {
    ...
    public static int getBucketCount(double width, double height, double areaPerBucket, int extraBucket) {
        if( width <= 0 || height <= 0 || areaPerBucket <= 0 || extraBucket < 0) {
            ...
            double bobHave = areaPerBucket * extraBucket;
            double bobNeed = Math.ceil((wallArea - bobHave)/areaPerBucket);
            return (int) bobNeed;
        }
    ...
    public static int getBucketCount(double width, double height, double areaPerBucket) {
        if( width <= 0 || height <= 0 || areaPerBucket <= 0) {
            ...
            double wallArea = width * height;
            double bobNeed = Math.ceil(wallArea /areaPerBucket);
            return (int) bobNeed;
    ...
    public static int getBucketCount(double area, double areaPerBucket) {
        if(area <= 0 || areaPerBucket <= 0) {
            ...
            double bobNeed = Math.ceil(area /areaPerBucket);
            return (int) bobNeed;
...
//
// *getBucketCount(3.4, 2.1, 1.5, 2); → should return 3 since the wall area is 7.14, a single bucket
// can cover an area of 1.5 and Bob has 2 extra buckets home.
//
// *getBucketCount(2.75, 3.25, 2.5, 1); → should return 3 since the wall area is 8.9375, a single
// bucket can cover an area of 2.5 and Bob has 1 extra bucket at home.
//
//
//
...
//
// *getBucketCount(3.4, 2.1, 1.5); → should return 5 since the wall area is 7.14, and a single
// bucket can cover an area of 1.5.
//
// *getBucketCount(7.25, 4.3, 2.35); → should return 14 since the wall area is 31.175, and a single
// bucket can cover an area of 2.35.
//
//
...
//
// Write another overloaded method named getBucketCount with 2 parameters namely, area and
// areaPerBucket (both of type double).
//
...
//
//
// NOTE: Use the method Math.ceil to round the number of calculated buckets (double) then convert it into an
// int before returning the value from the methods.
//
